#include <SFML/Graphics.hpp>
#include "core/game.h"
#include "ai/dqn.h"
#include <iostream>
#include <iomanip>
#include <sstream>
#include <vector>

static void draw_training_ui(sf::RenderWindow& window,
                              sf::Font& font,
                              int ep, int max_ep,
                              float avg_score, float epsilon) {
    window.clear(sf::Color(20, 20, 30)); // dark background

    // ── Title ─────────────────────────────────────────────────────────────────
    sf::Text title("AI Training in Progress", font, 32);
    title.setFillColor(sf::Color(100, 220, 100));
    title.setPosition(80, 60);
    window.draw(title);

    // ── Episode progress bar ───────────────────────────────────────────────────
    float bar_w = 640.f;
    float fill  = bar_w * ep / max_ep;

    sf::RectangleShape bar_bg(sf::Vector2f(bar_w, 24));
    bar_bg.setPosition(80, 130);
    bar_bg.setFillColor(sf::Color(60, 60, 80));
    window.draw(bar_bg);

    sf::RectangleShape bar_fill(sf::Vector2f(fill, 24));
    bar_fill.setPosition(80, 130);
    bar_fill.setFillColor(sf::Color(80, 160, 255));
    window.draw(bar_fill);

    // ── Stats text ────────────────────────────────────────────────────────────
    auto make_text = [&](const std::string& s, float x, float y,
                         sf::Color c = sf::Color::White) {
        sf::Text t(s, font, 24);
        t.setFillColor(c);
        t.setPosition(x, y);
        window.draw(t);
    };

    std::ostringstream oss;
    oss << "Episode:     " << ep << " / " << max_ep;
    make_text(oss.str(), 80, 180);

    oss.str("");
    oss << std::fixed << std::setprecision(2)
        << "Avg score:   " << avg_score << "  (last 50 episodes)";
    make_text(oss.str(), 80, 220);

    oss.str("");
    oss << std::fixed << std::setprecision(3)
        << "Epsilon:     " << epsilon;
    make_text(oss.str(), 80, 260,
              sf::Color(epsilon > 0.5f ? 255 : 100, 200, 100));

    make_text("Close window to stop training.", 80, 740, sf::Color(150, 150, 150));

    window.display();
}

int main() {
    sf::RenderWindow window(sf::VideoMode(800, 800), "Snake AI Training");

    sf::Font font;
    font.loadFromFile("sansation.ttf");

    game::GameController env(&window);
    dqn::DQNAgent agent(8, game::ACTION_COUNT);

    constexpr int MAX_EPISODES = 5000;
    constexpr int MAX_STEPS    = 1000;
    constexpr int PRINT_EVERY  = 50;

    std::vector<int> scores;
    float avg_score = 0.0f;

    for (int ep = 0; ep < MAX_EPISODES; ep++) {
        env.reset();
        auto state = dqn::normalize_state(env.AI_GetState());

        int  food_count = 0;
        bool done       = false;

        for (int step = 0; step < MAX_STEPS && !done; step++) {
            sf::Event event;
            while (window.pollEvent(event))
                if (event.type == sf::Event::Closed) { return 0; }

            int action = agent.select_action(state);
            auto [raw_next, reward, next_done] = env.AI_HeadlessStep(action);

            auto next_state = dqn::normalize_state(raw_next);
            agent.store(state, action, reward, next_state, next_done);
            agent.train_step();

            if (reward >= 10.0f) food_count++;
            state = next_state;
            done  = next_done;
        }

        scores.push_back(food_count);

        if ((ep + 1) % PRINT_EVERY == 0) {
            int   n = std::min(PRINT_EVERY, (int)scores.size());
            avg_score = 0.0f;
            for (int i = (int)scores.size() - n; i < (int)scores.size(); i++)
                avg_score += scores[i];
            avg_score /= n;

            std::cout << "Episode " << std::setw(5) << ep + 1
                      << "  |  Avg score: " << std::fixed << std::setprecision(2) << avg_score
                      << "  |  epsilon: " << std::setprecision(3) << agent.epsilon
                      << "\n";

            draw_training_ui(window, font, ep + 1, MAX_EPISODES,
                             avg_score, agent.epsilon);
        }
    }

    std::cout << "Training done.\n";
    return 0;
}
